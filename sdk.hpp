...
Camera get_view_point()
{
	...
	view_point.rotation.x = asin(tpmrotation.c) * (180.0 / M_PI);
	view_point.rotation.y = ((atan2(tpmrotation.a * -1, tpmrotation.b) * (180.0 / M_PI)) * -1) * -1;
	view_point.rotation.z = 0;
	view_point.location = read<Vector3>(location_pointer);
	view_point.fov = read<float>(cache::player_controller + 0x3AC) * 90.f;
	return view_point;
}
